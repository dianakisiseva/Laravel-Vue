            ...
            switch (_context.prev = _context.next) {
              case 0:
                ...
                if (!_this.user.password) {
                  _this.errors.push('Password required.');
                }
                _context.next = 5;
                return _this.axios.post('/login', _this.user).then(function (response) {
                  if (response.data.errors) _this.errors.push(response.data.errors);
                  if (response.data.status === 'success') _this.$router.push({
                    name: "taskList"
                  });
                });
              case 5:
